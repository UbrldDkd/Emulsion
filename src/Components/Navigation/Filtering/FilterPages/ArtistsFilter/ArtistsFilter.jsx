...
export default function ArtistsFilter() {
  ...
  const displayArtists = (() => {
    if (showOnlySelected) {
      return MOCK_ARTISTS.filter(artist => selectedArtists.has(artist.name));
    }
    return searchTerm
  ...
  const getSectionLabel = () => {
    if (showOnlySelected) return `SELECTED (${displayArtists.length})`;
    if (searchTerm) return `RESULTS (${displayArtists.length})`;
    return 'RECOMMENDED';
  };

  const getArtistNameColor = (isSelected) => {
    ...
    <div className="space-y-4 pb-4">
      <div className="relative">
        <div className="text-center pt-3 pb-6 px-4">
          <h3 className={`${theme.textSecondary} text-lg font-extralight tracking-[0.3em] uppercase mb-2`}>
            Magistri Artium
          </h3>
          <p className={`${theme.textMuted} text-xs font-light tracking-wide`}>
            Search visual artists
          </p>
        </div>
      ...
      <div className="max-h-48 overflow-y-auto space-y-1 scrollbar-hide">
        <div className={`text-xs mb-3 font-medium ${isLightMode ? 'text-stone-600' : 'text-stone-300'}`}>
          {getSectionLabel()}
        </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <div className={`font-medium text-sm ${getArtistNameColor(isSelected)}`}>
                    {artist.name}
                  </div>
                  <div className={`text-xs mt-0.5 ${isLightMode ? 'text-stone-600' : 'text-stone-300'}`}>
      ...
      {selectedArtists.size > 0 && (
        <div className={`flex items-center justify-between ${theme.cardBackground} ${theme.border} px-3 py-2`}>
          ...
          >
            <span className="bg-amber-200/30 text-amber-950 font-medium px-1.5 py-0.5 rounded-full">
              {selectedArtists.size}
            </span>
            <span className={isLightMode ? 'text-stone-600' : 'text-stone-300'}>
              selected
            </span>
            <svg
